...
- For a database that already has `alembic_version` set, run
  `alembic upgrade head` as part of your deploy after pulling a new release
  that adds migrations, so new columns/tables/indexes are applied before the
  app starts.
